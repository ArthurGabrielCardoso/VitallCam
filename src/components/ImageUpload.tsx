...
export default function ImageUpload({ patientId, onUpload, className = '', folderId = null, label, iconClassName = 'w-6 h-6' }: ImageUploadProps) {
  ...
  return (
    <>
      ...
      <label
        htmlFor={inputId}
        className={`inline-flex items-center justify-center gap-2 cursor-pointer bg-secondary hover:bg-secondary/90 text-white ${className} ${createFolderMutation.isPending ? 'opacity-50 pointer-events-none' : ''}`}
      >
